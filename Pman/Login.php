...
//        exit;
    ...
    function post()
    {
        ...
        if (!empty($ff->Pman['auth_comptype']) && $ff->Pman['auth_comptype'] != $u->company()->comptype) {
            $this->jerr("Login not permited to outside companies"); // serious failure
        }
        ...
        $this->jerror('BADPASS', 'You typed the wrong Username or Password  (2)'); // - " . htmlspecialchars(print_r($_POST,true))."'");
        exit;
    }
